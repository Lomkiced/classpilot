    ...
    <aside className={`fixed inset-y-0 left-0 flex flex-col border-r border-gray-200 bg-white transition-all duration-300 z-20 ${widthClass}`}>
            ...
            {group.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group relative flex items-center rounded-xl px-3 py-2.5 text-[14px] font-medium transition-all ${
                ...
                >
                  ...
                  <item.icon
                    className={`shrink-0 transition-colors ${
                      mounted && isCollapsed ? "h-6 w-6" : "h-[20px] w-[20px]"
                    } ${
      ...
      <div className="border-t border-gray-100 p-4 shrink-0">
        <button
          ...
          className={`group flex w-full items-center rounded-xl px-3 py-2.5 text-[14px] font-medium text-gray-500 transition-colors hover:bg-red-50 hover:text-red-700 ${
            mounted && isCollapsed ? "justify-center" : "justify-start gap-3.5"
          }`}
        >
          <LogOut
